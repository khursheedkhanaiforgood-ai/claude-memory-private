...
## Sept 7 2026 (later same day) — Port 1/11 saga RESOLVED via independent transcript review; two open items retired

User fed in a separate 62-page Claude transcript (`Fabric-Connect-Lab-Full-Transcript_Sep 7 2026.pdf`, a parallel session run outside this one) documenting the actual live continuation and resolution of the port 1/11 puzzle above. Full review delivered; key findings that update this file directly:

**Port 1/11 was never a switch/fabric fault — root cause was the EP1/Platform ONE onboarding chain.** `show fa` confirmed port 1/3 (working) = `ELEM AUTH: AP, ASGN AUTH: AP` vs. port 1/11 = `ELEM AUTH: AP, ASGN AUTH: none` — the switch correctly recognized the AP as an FA client every time; it simply never received a second-stage VLAN154→I-SID144154 assignment *request* from the AP, because the AP never got that far in its own cloud onboarding. Traced chain: AP stuck at 34% → CAPWAP/redirector reachability suspected → **Reset to Default in Platform ONE + re-onboard via Local Management** fixed the CAPWAP/DTLS handshake (`RUN state: Connected securely`, 0 keepalives lost) → but device still wasn't reaching **Managed** state in EP1 (separate gate: license pool + OS/persona finalization) → discovered EP1 wasn't showing the device *at all* → traced to the AP's serial number being orphaned from EP1's inventory after the factory reset (reset makes the device "reconnect as new") → fixed via delete-and-re-add in Subscriptions & Services → Inventory → even after that, policy still wasn't pushing, isolated to one of four distinct Platform ONE device states (Configuration Audit Mismatch / Configuration at Device Level override / Configuration Pending / Device Update Unsuccessful — check the "Updated" column hover tooltip for the exact one). **Physical/L2 layer (PoE, port speed/duplex, FDB entry in VLAN153) was clean throughout — confirms this was never a cabling or AP-hardware fault**, ending the "cable-swap vs AP-side check" open question from Sept 5.

**Confirms, from a live `sw1_capture_20260905.log` capture quoted directly in that transcript: the AP's own management I-SID is 144153, not 144154.** `auto-sense fa wap-type1 i-sid 144153` is what's actually live on SW1 — this resolves item 2 from the Sept 5 open-items list ("143154 vs 144153 typo theory") as **confirmed correct**, no longer just inferred.

**New topology detail:** two more named switches appear as next-hops in SW1's route table — `TSLab-SW-01` and `Fishbowl-SW-01` — meaning the live fabric extends beyond just SW1/SW2/RDU-Core. Not yet explored further.

**Also confirmed live:** manually running `fa enable` on an auto-sense port is explicitly rejected — `Error: port 1/11, Manual configuration not allowed on Auto-Sense port` — immediately followed by unplug/replug letting auto-sense handle discovery correctly (LLDP neighbor discovered → FA element discovered as clientWapType1 → element auth succeeded). Confirms ports must stay on factory auto-sense defaults; manual `fa enable`/`isis` config on AP-facing ports is not just unnecessary but actively rejected by the switch.

**Note — that same transcript's pages 1-11 cover a separate, unrelated lab** ("vBOB1-Headend Lab": a Primary Fabric Switch + branch switches + Fabric Extend over an SD-WAN/IPE appliance to `RDU-Core`/`FishBowl-Fabric_Core-5720-01`). Don't conflate this with KhKLab-SW-01/SW2 — it's parallel background work with its own config review thread (several syntax bugs found and fixed: missing `portmember` keyword, `fa port-enable` vs `interface .../fa enable` confusion, a missing `exit` in a Flex-UNI block, a missing `next-hop` keyword on an `ip route` line).

**Remaining open items after this review:** the AP flap's ultimate trigger (why 1/7↔1/11 bounced in the first place) was never separately diagnosed — it's subsumed by the onboarding-chain root cause above, but worth noting the flap itself stopped being relevant once the EP1 chain was identified as the actual blocker. `sw1_capture_20260905.log`'s abrupt ending remains unexplained but is no longer load-bearing for this issue. Item 1 (L3 VSN/IPVPN, queued in `project_l3vsn_grt_deepdive.md`) was also substantially covered in this same transcript — see that file's update.

EOD: `lab-journal/khklab_sw1_ap1-11_diagnosis_20260905.html` (pending push).

## Sept 7 2026 (still later) — SW1/SW2 lab build reference cross-checked against production capture, two rounds of review, `vlan members add` KB conflict closed

A parallel Claude session (same transcript source as above) pulled the real production SW1 config out of `sw1_capture_20260905.log` and diffed it against the SW1/SW2 lab build reference doc (`SW1-SW2-Fabric-Connect-Lab-Build-Reference*.pdf`, a companion to the `beb_build_reference_20260905.html` doc). Two review rounds happened, both fed to this session for independent audit:

**Round 1 fixes (self-consistent, verified against the PDF):** all `type port` VLAN-creation lines corrected to `type port-mstprstp 0` (10 occurrences across both switches), all `ip address` lines on loopback/VLAN interfaces got a numeric index parameter added (7 occurrences: loopback uses a **leading** index, VLAN interfaces use a **trailing**, incrementing index — both are real, distinct positions in the grammar), and every `vlan members add <vid> <port>` line was changed to `vlan members <vid> <port> portmember` (2 occurrences), dropping the `add` keyword, on the theory that the production capture's exact quote (`vlan members 153 1/5 portmember`) was ground truth.

**This session's audit flagged two real conflicts** between that "no add" change and our own switch-confirmed `reference_voss_cli.md` (which has `vlan members add 1 1/1` tested live on the actual 5320, Apr 30 2026): (1) the `vlan members` syntax question — running-config dumps can show a normalized/canonical form different from what's actually typed, so a bare capture-file quote isn't proof of interactive syntax; (2) the `ip address` index — `reference_voss_cli.md` confirmed the plain form (no index) also works live, raising the question of whether the index is optional.

**Round 2 (fresh KB check, not just citation defense) settled both cleanly:**
- **`ip address` index is optional** — confirmed via Fabric Engine 9.3 Command Reference/User Guide grammar: `ip address {<A.B.C.D/X> | <A.B.C.D> <A.B.C.D>} [<0-511>] [dvr-one-ip] [name WORD] [secondary] [state-disabled]` — square brackets = optional. Both the indexed and un-indexed forms are valid; production just happened to write it explicitly. **No change needed** — left as-is in the reference doc.
- **`vlan members add` is NOT optional** — the documented grammar is `vlan members add <1-4059> {port-list} [{portmember|static|notallowed}]` — `add` is part of the verb itself; only the trailing qualifier is bracketed/optional. This confirms `reference_voss_cli.md`'s live-tested form was right all along, and the "no add" edit in round 1 was a mistake — **reverted**. The production capture's `vlan members 153 1/5 portmember` (no `add`) is most likely a running-config dump showing the switch's normalized stored form, not what you'd actually type interactively — exactly the theory this session proposed, now KB-confirmed rather than guessed.
- **`lease-time <60-300000000>` confirmed** as a real, documented DHCP server sub-command (same KB source) — was flagged as unverified in the prior review, now closed, no change needed.

**This closes the "unresolved `vlan members add` vs `vlan member add` KB conflict (verify on-switch)" item** originally flagged Sept 5 in the `beb_build_reference_20260905.html` review (see MEMORY.md's now-stale note on that file). The grammar question is settled by documented syntax; the only remaining step is confirming it live on KhKLab-SW-01/SW2 itself, which hasn't happened yet.
