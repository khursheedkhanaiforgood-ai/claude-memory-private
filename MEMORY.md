...
- **Fabric Extend (FE) concept study (Sept 1) → SUPERSEDED by live hardware session (Sept 2).** See next bullet for current state.
- **LIVE HARDWARE SESSION — KhKLab-SW-01, Sept 2, IN PROGRESS.** FE (port 1/1→SD-WAN→RDU-Core), Auto-Sense NNI (port 1/10→SW2), and Fabric Attach (ports 1/3, 1/7→AP3000s) all confirmed simultaneously working via `show interfaces gigabitethernet auto-sense`/`show isis adjacencies`/`show isis lsdb`/`show fa assignment`. **Two open issues:** (1) 1/3 and 1/7 landed on two different I-SIDs (143154 vs 144154) for the same VLAN 154 — waiting on user to confirm if shared-VSN or intentional isolation was intended. (2) AP on port 1/7 just went **down** (link-state, not FA) — not yet diagnosed, no commands run. Full log: `project_voss_fabric_migration.md` → "LIVE HARDWARE SESSION, Sept 2" section.
- **Fabric Connect teaching arc (Sept 5) — L2 VSN/802.1ah/BEB-BCB done. Next session: L3 VSN + GRT/IP Shortcuts deep-dive.** Context: `project_l3vsn_grt_deepdive.md`. EOD: https://github.com/khursheedkhanaiforgood-ai/EOD_HTML → `session_summary_20260905.html`.
- **SW2 BEB build reference DONE, pushed (Sept 5, commit `9d5a3ce`)** — `beb_build_reference_20260905.html` in EOD_HTML repo: 7-module CLI structure (placeholders, user writes actual values) for building SW2 from factory-reset into a BEB aligned with SW1's live fabric. KB-audited: fixed missing `fa enable` prereq, flagged unresolved `vlan members add` vs `vlan member add` KB conflict (verify on-switch), added low-effort `auto-nni`+Fabric Attach framing. User is reviewing after a break — still open: the 143154 vs 144154 I-SID decision (same as Sept 2 bullet above) gates Module 7.
...
- **EOD Journal repo:** `/Users/khukhan/lab-journal/` — pushed to GitHub (private) as `khursheedkhanaiforgood-ai/EOD_HTML` on Sep 5 2026. EOD files: `session_summary_YYYYMMDD.html`.
...
- [project_voss_fabric_migration.md](project_voss_fabric_migration.md) — EXOS→VOSS migration: D1–D9 complete (Aug 20). **HOME LAB STARTS 2026-08-21 MORNING** — two 5320s (fabric_se VOSS + Exos_EP1 convert). First: backup EXOS → convert OS → configure both via SE+EP1 → cable NNI last. CLI scripts in file.
- [project_voss_d1_session_20260820.md](project_voss_d1_session_20260820.md) — D1 Socratic session Aug 20: Four Laws, B-MAC/C-MAC, B-FIB/Service FIB/I-SID Membership, BEB/BCB. All corrections logged. Ready for D2.
- [project_voss_siteengine_lab.md](project_voss_siteengine_lab.md) — BLOCKED: Site Engine voss_policy + I-SID for AP traffic. Waiting on Karl to fix EVE-NG disk → IS-IS adjacency. Resume when ISIS Adjacency alarm clears.
...
- [feedback_eth0_uplink_ep1_trunk.md](feedback_eth0_uplink_ep1_trunk.md) — AP connectivity issue root cause: ETH0 uplink set to Trunk in device-level EP1 setting (not switch-side). Resolved 2026-09-01.
